/*
* \file process5.cpp
* \brief process events using Alg 2 + shuffling
...
// Alg 2 + shuffling: Split events into sub-intervals, shuffle mini-DSIs across time and cameras, fuse them
...
//    }
  ...
  // When shift=0, there is no shuffling
  int shift = num_subintervals/2;

  // Fuse the DSIs
  mapper_fused.dsi_.resetGrid();
  unsigned int idx_first_ev[2] = {0, shift*num_events_per_subinterval[1]};
#ifdef TIMING_LOOP
  int nloops = 100;
#endif
  ...
  for (unsigned int k=0; k < num_subintervals; k++)
    {
      ...
      {// Left camera:
        ...
        //        LOG(INFO)<< idx_first_ev[0] << "-" << idx_first_ev[0] + num_events_per_subinterval[0];
      ...
      {// Right camera:
        // Approximation: this selection does not guarantee that events belong to the same time interval on both cameras
        std::vector<dvs_msgs::Event> events_subset;
        if (idx_first_ev[1] + num_events_per_subinterval[1] >= events1.size()){

            //            LOG(INFO)<< idx_first_ev[1] << "-" << idx_first_ev[1] + num_events_per_subinterval[1] - events1.size();
            events_subset = std::vector<dvs_msgs::Event>(events1.begin() + idx_first_ev[1],
                events1.end());
            events_subset.insert(events_subset.end(), events1.begin(), events1.begin() + idx_first_ev[1] + num_events_per_subinterval[1] - events1.size());
            idx_first_ev[1]=idx_first_ev[1] + num_events_per_subinterval[1] - events1.size();
          }
        else{

            //            LOG(INFO)<< idx_first_ev[1] << "-" << idx_first_ev[1] + num_events_per_subinterval[1];
            events_subset = std::vector<dvs_msgs::Event>(events1.begin() + idx_first_ev[1],
        ...
        {// Display accumulted events to visualize the amount of intra-camera shift/baseline
          ...
          accumulateEvents(events_subset,true,event_image);
          std::stringstream filename;
          filename << "events_1_" << std::setfill('0') << std::setw(3) << (k+shift)%num_subintervals << ".png";
          cv::imwrite(out_path+filename.str(),event_image);
        }

        // Back-project events into the DSI
        mapper1.evaluateDSI(events_subset, trajectory1, T_rv_w);
        LOG(INFO) << "Right: Sub-interval " << (k+shift)%num_subintervals << "  Mean square = " << mapper1.dsi_.computeMeanSquare();

        //        mapper1.getDepthMapFromDSI(depth_map, confidence_map, semidense_mask, opts_depth_map);
        //        {
        //          std::stringstream ss_suffix;
        //          ss_suffix << "1_" << std::setfill('0') << std::setw(3) << (k+shift)%num_subintervals;
        //          saveDepthMaps(depth_map, confidence_map, semidense_mask, dsi_shape.min_depth_, dsi_shape.max_depth_, ss_suffix.str(), ss.str());
        //        }
      }
